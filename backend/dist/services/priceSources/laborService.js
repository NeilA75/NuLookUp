...
exports.getLaborServicePrices = getLaborServicePrices;
 ...
 */
async function fetchFromBLS(occupation) {
    try {
        // Note: BLS API requires series ID. This is a simplified example.
        // In production, map occupation to correct BLS series ID
        const response = await axios_1.default.get('https://api.bls.gov/publicAPI/v2/timeseries/data/', {
            method: 'POST',
        ...
        const prices = data.map((d) => parseFloat(d.value || '0')).filter((v) => v > 0);
        if (prices.length === 0) {
            return [];
        }
 ...
 */
async function scrapePayScale(query) {
 ...
 */
async function scrapeGlassdoor(query) {
    const page = await (0, scraperClient_1.getPage)();
    try {
 ...
 */
async function getLaborServicePrices(query) {
    try {
