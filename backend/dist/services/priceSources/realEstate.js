...
exports.getRealEstatePrices = getRealEstatePrices;
 ...
 */
async function scrapeZillow(query) {
    ...
    try {
        ...
        const listings = await page.evaluate(() => {
            return Array.from(document.querySelectorAll('[data-testid="property-card"]'))
                ...
                .filter((p) => p !== null && p > 10000) // Reasonable minimum
                .slice(0, 100);
        });
...
async function scrapeRedfin(query) {
    const page = await (0, scraperClient_1.getPage)();
    try {
        const location = query.replace(/\b(house|home|apartment|condo|property|real estate)\b/gi, '').trim();
 ...
 */
async function getRealEstatePrices(query) {
    try {
        // Try Zillow first
