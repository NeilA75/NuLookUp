...
// Hash any string to one of 8 distinct color palettes
const COLOR_PALETTES = [
  { bg: 'rgba(56,189,248,0.12)',  color: '#38bdf8', border: 'rgba(56,189,248,0.3)'  }, // sky
  { bg: 'rgba(251,191,36,0.15)',  color: '#fbbf24', border: 'rgba(251,191,36,0.3)'  }, // amber
  { bg: 'rgba(99,102,241,0.15)',  color: '#818cf8', border: 'rgba(99,102,241,0.3)'  }, // indigo
  { bg: 'rgba(52,211,153,0.15)',  color: '#34d399', border: 'rgba(52,211,153,0.3)'  }, // emerald
  { bg: 'rgba(251,146,60,0.15)',  color: '#fb923c', border: 'rgba(251,146,60,0.3)'  }, // orange
  { bg: 'rgba(232,121,249,0.15)', color: '#e879f9', border: 'rgba(232,121,249,0.3)' }, // fuchsia
  { bg: 'rgba(163,230,53,0.15)',  color: '#a3e635', border: 'rgba(163,230,53,0.3)'  }, // lime
  { bg: 'rgba(251,113,133,0.15)', color: '#fb7185', border: 'rgba(251,113,133,0.3)' }, // rose
]

function getTypeStyle(type: string) {
  // Deterministic hash so same type always gets same color
  ...
  return (
    <motion.div
      ...
      whileHover={{
        ...
        boxShadow: `0 0 30px ${typeStyle.bg}, 0 8px 32px rgba(0,0,0,0.4)`,
        y: -2,
      }}
    >
      <div
        ...
        style={{
          background: `radial-gradient(circle, ${isPositive ? 'rgba(16,185,129,0.2)' : 'rgba(239,68,68,0.15)'} 0%, transparent 70%)`,
        }}
      />

      <div className="flex justify-between items-start">
        <div>
          <h3 className="text-lg font-bold text-slate-200" style={{ fontFamily: "'IBM Plex Mono', monospace" }}>
            {item.title}
          </h3>
        ...
        <div className="flex flex-col items-end shrink-0 ml-4">
          ...
          <span className="text-lg font-bold text-slate-100" style={{ fontFamily: "'IBM Plex Mono', monospace" }}>
            {item.value}
          </span>
          <span
            className={`text-sm font-semibold mt-0.5 ${isPositive ? 'text-emerald-400' : 'text-red-400'}`}
